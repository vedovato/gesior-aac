...
if(!defined('INITIALIZED'))
	exit;
																	...
																	$main_content .= '
																		<div class="ServiceID_Icon_Container" id="ServiceID_Icon_Container_'.$product['id'].'">
																			...
																			if ($product['category'] == 6) {
																				$main_content .= '
																					<div class="ServiceID_Icon_New" id="ServiceID_Icon_New_'.$product['id'].'" style="background-image:url('.$layout_name.'/images/shop/coins.gif); background-repeat:no-repeat; margin:20px 45px;" ></div>';
																			}
																		...
																		<div class="PMCID_Icon" id="PMCID_Icon_1" style="background-image:url('.$layout_name.'/images/payment/pmcid_icon_normal.png);" onclick="ChangePMC(1);" onmouseover="MouseOverPMCID(1);" onmouseout="MouseOutPMCID(1);" >
																			...
																			<span style="position: absolute; left: 125px; top: 53px; z-index: 99;" >
																				<span style="margin-left: 5px; position: absolute; margin-top: 2px;" >
																					<span class="HelperDivIndicator" onMouseOver="ActivateHelperDiv($(this), \'Information:\', \'Coins can be used to purchase Extra Services, Mounts, Outfits and Items.\', \'\');" onMouseOut="$(\'#HelperDivContainer\').hide();" >
																						<image style="border:0px;" src="'.$layout_name.'/images/global/content/info.gif" />
																					</span>
																				</span>
																			</span>
																			<img class="PMCID_CP_Icon" src="'.$layout_name.'/images/payment/coins.gif" />
																			<div class="PMCID_CP_Label" >
																				<input type="radio" id="PMCID_1" name="PMCID" value="1">
																				<label for="PMCID_1" >Coins</label>
																			</div>
																		</div>
																	</div>
																</div>
		...
		if($step == 2) {
			...
			if(empty($services_errors)) {
				...
				if($account_logged->getPremiumPoints() < $service_points)
					$services_errors[] = "You need at least ".$service_points." coins to purchase the ".$service_name.".";
			}
			if(empty($services_errors))
															...
															<div class="TableContentContainer" >
																<table class="TableContent" width="100%"  style="border:1px solid #faf0d7;" >
																	...
																	<tr>
																		...
																		<td>' . $service_info['coins'] . ' Coins</td>
																	</tr>';
															$main_content .= '
																</table>
															</div>
		...
		if($step == 4) {
			...
			if($account_logged->getPremiumPoints() < $service_points)
				$services_errors[] = "You need at least ".$service_points." coins to purchase it.";
			...
			if(empty($services_errors)) {
				if( $_SERVER['REQUEST_METHOD']=='POST' ) {
					...
					if( isset( $_SESSION['hash'] ) && $_SESSION['hash'] == $hash ) {
						// Refresh! Does nothing or re-display the completed form
					} else {
