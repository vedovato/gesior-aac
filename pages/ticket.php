...
if ($action == "createticket") {
	$categories = [
		1 => 'Help', 2 => 'Donate', 3 => 'Suggestions', 4 => 'Report Bug', 5 => 'Claims',
	...
	$insert = $SQL->query("INSERT INTO `tickets`(`ticket_subject`, `ticket_author`, `ticket_author_acc_id`,`ticket_last_reply`, `ticket_admin_reply`,`ticket_date`, `ticket_ended`, `ticket_status`, `ticket_category`, `ticket_description`) VALUES (" . $SQL->quote($subject) . ", " . $SQL->quote($playerName) . ", " . $SQL->quote($accid) . ", 'You', 0, " . $SQL->quote($date) . ", 'Not closed', 'Waiting', " . $SQL->quote($category) . ", " . $SQL->quote($description) . ")");
	// $insert->execute(['subject' => $subject, 'playerName' => $playerName, 'accid' => $accid, 'date' => $date, 'category' => $category, 'description' => $description]);
	$generateId = $SQL->query("SELECT ticket_id AS 'id' from `tickets` WHERE ticket_author_acc_id = " . $SQL->quote($accid) . " ORDER BY `id` DESC")->fetchAll()[0]['id'];
	$main_content .= '
	...
	if ($metodo == 'reply') {
		...
		if (strlen($mensagem) < 10 || strlen($mensagem) > 1000) {
			...
			if ($replyAuthorId == $account_logged->getID()) {
				...
				// echo $group_id_of_acc_logged . "-----------------" . $account_logged->getName() . "0--------------------0". $config['site']['access_admin_panel'];
				if ($group_id_of_acc_logged >= $config['site']['access_admin_panel']) {
					$players_from_logged_acc = $account_logged->getPlayersList();
					foreach ($players_from_logged_acc as $player) {
						// echo $player->getGroupID() . "<br/>";
						if ($player->getGroupID() >= $config['site']['ticket_reply_access']) {
